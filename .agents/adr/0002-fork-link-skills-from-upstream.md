...
Upstream's version scans only `skills/` and links into `~/.claude/skills` + `~/.agents/skills` as symlinks. Ours scans the **whole repo root** (so the `android-*` and `gcp-*` skill dirs that `update-skills.sh` syncs into `skills/android/` and `skills/gcp/` get linked at all), links into `~/.claude/skills` + `~/.cursor/skills`, copies **real dirs** for Cursor (its slash-command picker won't follow symlinks), and prunes stale/deprecated/orphan entries.

Adopting upstream's would silently stop linking the synced android/gcp skills and drop Cursor support; we use `~/.cursor` and not `~/.agents`. So we keep ours.
