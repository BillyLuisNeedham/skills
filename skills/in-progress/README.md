...
- **[sitrep](./sitrep/SKILL.md)** — Dispatch a subagent to build a visual sitrep of the work in flight — mission, situation, next move — as a self-contained HTML page that opens in the browser. Briefed from the working agent's memory. Model-invoked, so other skills can open a session with one.
- **[sitrep-figure-it-out](./sitrep-figure-it-out/SKILL.md)** — The same sitrep, reconstructed from the session transcript on disk instead of briefed from memory, so the working agent spends almost nothing. Shares `sitrep`'s report guide. User-invoked.
